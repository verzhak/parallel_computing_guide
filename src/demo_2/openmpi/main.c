...
Запуск программы:
	...
	mpirun -mca orte_rsh_agent rsh -n 4 -H localhost,192.168.0.188 -loadbalance -x PATH="/bin:/usr/bin:/sbin:/usr/sbin:/usr/local/bin:/usr/local/sbin:`pwd`:/home/amv/trash/" demo

	Здесь:

		-mca orte_rsh_agent rsh	- предписание использовать протокол RSH для пересылки сообщений между вычислительными потоками;
		-n 4 - предписание запустить четыре вычислительных потока;
		-H localhost,192.168.0.188 - перечисление адресов (символьных и IP-адресов) вычислительных систем, в которых будут запущены вычислительные потоки;
		-loadbalance - предписание равномерно распределить нагрузку между вычислительными системами;
...
#define NUM_TAG 1
	...
	if(t_ind)
	{
		...
		/* Получение вычислительным потоком сообщения с тегом, равным значению константы компилятора NUM_TAG - получение вычислительным потоком сообщения, содержащего количество реализаций случайной величины с гамма - распределением, генерируемых данным вычислительным потоком */
		...
		{
			/* Генерация очередной реализации случайной величины */
			for(v = 0, P = 0; v < alpha; v++)
				P += exp_distrib(beta);

			/* Подсчет суммы реализаций случайной величины */
			r_m += (x[u] = P);
		}
	}
	else
		...
		{
			/* Вычисление и отправление каждому вычислительному потоку количества генерируемых им реализаций случайной величины */
	...
	/* Сбор сумм сгенерированных вычислительными потоками реализаций случайной величины */
	call(MPI_Gather(& r_m, 1, MPI_DOUBLE, m, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD));

	if(! t_ind)
	{
		/* Главный вычислительный поток (поток с номером 0) рассчитывает эмпирическое математическое ожидание случайной величины */
	...
	if(t_ind)
		/* Вычислительные потоки с номерами, большими нуля, рассчитывают суммы квадратов центрированных реализаций случайной величины */
	...
	/* Сбор сумм квадратов центрированных реализаций случайной величины */
